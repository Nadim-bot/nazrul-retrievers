...
export default function SearchHistoryChart() {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6 transition-all">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5 pb-4 border-b border-slate-200">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#0A192F] text-amber-400 flex items-center justify-center shadow-sm shrink-0 border border-slate-800">
            <Search className="w-5 h-5 stroke-[2.5]" />
          </div>
          <div>
            <h4 className="font-serif text-lg font-black text-[#0A192F] tracking-tight">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={loading}
            className="px-3.5 py-2 bg-[#0A192F] hover:bg-[#1E293B] text-amber-300 rounded-xl font-bold text-xs transition-all flex items-center gap-1.5 cursor-pointer shadow-sm active:scale-95"
          ...
          <button
            onClick={handleReset}
            className="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-[#0A192F] rounded-xl font-bold text-xs transition-all cursor-pointer active:scale-95 border border-slate-300"
      ...
      <div className="flex items-center gap-2 mb-5 flex-wrap">
        <span className="text-xs font-black uppercase tracking-wider text-[#0A192F] mr-1">
          Filter by:
        </span>
        {categories.map(cat => {
          ...
          return (
            <button
              ...
              className={`px-3.5 py-1.5 text-xs font-extrabold rounded-xl transition-all cursor-pointer ${
                isActive
                  ? 'bg-[#0A192F] text-amber-300 shadow-md scale-105'
      ...
      <div className="relative w-full overflow-hidden bg-slate-50 rounded-2xl p-4 border border-slate-200">
        <div className="h-[280px] w-full">
          {loading ? (
            <div className="h-full flex flex-col items-center justify-center text-[#0F172A] gap-2 p-6 text-center">
              <RefreshCw className="w-6 h-6 animate-spin text-amber-600" />
              <span className="text-sm font-bold text-[#0F172A]">Loading real-time search trends...</span>
            </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-[#0F172A] gap-2 p-6 text-center">
              ...
              <p className="text-xs font-medium text-[#475569] max-w-md">
                {filterCategory === 'All'
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#E2E8F0" opacity={0.8} />
                <XAxis 
                  type="number" 
                  fontSize={11} 
                  fontWeight={800} 
        ...
        {filteredData.length > 0 && (
          <div className="absolute bottom-2 right-3 text-xs text-[#0F172A] font-black flex items-center gap-1.5">
      ...
      <div className="mt-5 bg-amber-50/80 border border-amber-200 rounded-2xl p-4 sm:p-5 shadow-sm flex items-start gap-3.5">
        <div className="w-9 h-9 rounded-xl bg-amber-500/20 flex items-center justify-center text-amber-800 shrink-0 mt-0.5 border border-amber-500/30">
          <AlertCircle className="w-5 h-5" />
        </div>
        <div>
          <span className="font-black uppercase tracking-widest text-xs text-amber-900 block mb-1">
            System Insight & Recommendations
          </span>
          {historyData.length === 0 ? (
            <p className="text-[#0F172A] text-xs sm:text-sm leading-relaxed font-bold">
            ...
            <p className="text-[#0F172A] text-xs sm:text-sm leading-relaxed font-bold">
              <strong className="text-[#0F172A] font-black">{historyData[0]?.keyword}</strong>
              {historyData[1] && (
                <>
